...
func _dump_social_graph() -> void:
	...
	var npc_headers = ["ID", "姓名", "境界", "阵营", "性格标签", "状态", "财富", "世界坐标"]
	var npc_rows = []
	for id in sm.npc_attributes:
		...
		var pos_str = "(%.1f, %.1f, %.1f)" % [data.world_position.x, data.world_position.y, data.world_position.z]
		npc_rows.append([
			id, 
			data.npc_name, 
	...
	log_table(8, npc_headers, npc_rows)
