...
func _ready():
	hide()
	color = Color(0.05, 0.08, 0.06, 0.95)
	anchors_preset = 15
	anchor_right = 1.0
	anchor_bottom = 1.0
...
func _refresh_all():
	...
	if player_comp:
		...
		for slot in player_comp.slots:
			if slot:
				p_inv[slot.id] = p_inv.get(slot.id, 0) + slot.qty
...
func _recalc_diff():
	var diff = 0
	for id in staged_sell: diff += ItemDatabase.get_item(id).get("price", 10)
	for id in staged_buy: diff -= ItemDatabase.get_item(id).get("price", 10)
