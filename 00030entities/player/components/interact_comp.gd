...
func handle_interact_only() -> bool:
	print("\n--- [DEBUG F键交互链路开始] ---")
	if not player: 
...
func _physics_process(delta: float) -> void:
	...
	if stats:
		...
		# 强制把交互射线加长到 2.5 米 (适合第一人称交互的范围)
		interaction_ray.target_position = Vector3(0, 0, -2.5)
	
	interaction_ray.force_raycast_update()
	var main_hud = player.get_node_or_null("HUD/MainHUD")
	if main_hud and main_hud.has_method("set_interaction_prompt"):
		if interaction_ray.is_colliding():
			var collider = interaction_ray.get_collider()
...
func try_interact() -> bool:
	print("[DEBUG] InteractComponent 收到交互判定请求")
	return handle_interact_only()
