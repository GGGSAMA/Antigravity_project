...
signal cast_started(spell_name: String, time_needed: float)
signal cast_updated(time_passed: float)
signal cast_ended()
signal notification_requested(msg: String)

var is_charging: bool = false
var charge_is_left: bool = false
var charge_is_item: bool = false
var current_action_data: SpellData = null
...
func _process(delta: float) -> void:
	if is_charging:
		...
		var total_time = current_action_data.cast_time
		var is_continuous = current_action_data.is_continuous

		cast_updated.emit(charge_time_passed)

		if is_continuous:
			# 持续施法逻辑：按住不放时，每隔 cooldown 时间自动触发一次法术
			continuous_cast_timer += delta
			var cooldown = current_action_data.cooldown
...
func execute_attack_from_state(is_left: bool) -> void:
	if is_left:
		handle_left_click(true)
	else:
		handle_right_click(true)

# ------------------------------------------------------------------------------
# 处理玩家左键输入
# 优先级：1. 快捷栏物品 > 2. 武器物理挥砍 > 3. 左手法术
...
func handle_left_click(pressed: bool = true) -> void:
	...
	if pressed:
		...
		if hotbar and hotbar.get("active_slot_index") != null:
			var idx = hotbar.get("active_slot_index")
		...
		if weapon != null:
			...
			if has_node("/root/Log"):
				get_node("/root/Log").info("Combat", "左键：没有装备武器，执行空手攻击")
			var vm = player.get("viewmodel")
			if vm and vm.has_method("trigger_left_swing"): 
				vm.trigger_left_swing()
			return
...
func handle_right_click(pressed: bool = true) -> void:
	...
	if pressed:
		if has_node("/root/Log"):
			get_node("/root/Log").info("Combat", "右键按下，尝试使用特殊物品或施展法术")
		...
		if active_item != null:
			# TODO: 接入右键特殊物品使用逻辑
			var vm = player.get("viewmodel")
			if vm and vm.has_method("trigger_right_swing"):
		...
		if has_node("/root/Log"):
			get_node("/root/Log").info("Combat", "右键松开，取消特殊物品使用或判断法术释放")
...
func _start_charge(is_left: bool) -> void:
	...
	if has_node("/root/Log"):
		get_node("/root/Log").debug("Combat", "选定法术: " + current_action_data.name)

	var stats = player.get("stats")
	var base_mana_cost = current_action_data.mana_cost
	var element = current_action_data.effect_type if current_action_data.effect_type != "" else "fire"
	...
	if not spell_comp.can_cast(is_left, current_mana):
		...
		notification_requested.emit("空的！")
		return
	...
	var base_cooldown = current_action_data.cooldown
	...
	if vm and vm.has_method("start_casting_animation"):
		var c = Color(current_action_data.color) if current_action_data.color != "" else Color.WHITE
		vm.start_casting_animation(is_left, c)

	var time_needed = current_action_data.cast_time
	var spell_name = current_action_data.name
	if has_node("/root/Log"):
		get_node("/root/Log").info("Combat", "开始蓄力法术：" + spell_name + " 需要时间：" + str(time_needed))

	if time_needed > 0.0:
		cast_started.emit(spell_name, time_needed)

# ==========================================
# 【施法就绪提示接口】(VFX / SFX)
# ==========================================
func _play_cast_ready_sfx(spell_data: SpellData) -> void:
	# 预留给音频管理器的接口：
	# var audio_manager = get_node_or_null("/root/AudioManager")
	# if audio_manager: audio_manager.play_sfx("spell_ready")
	if has_node("/root/Log"):
		get_node("/root/Log").debug("Combat", "【接口】播放蓄力完成音效：" + spell_data.name)

func _play_cast_ready_vfx(is_left: bool, spell_data: SpellData) -> void:
	...
	if has_node("/root/Log"):
		get_node("/root/Log").debug("Combat", "【接口】播放蓄力完成特效：" + spell_data.name)
...
func _release_charge() -> void:
	...
	if not is_continuous:
		if charge_time_passed >= total_time:
			...
			notification_requested.emit("施法被打断！")
...
func apply_hitbox_damage() -> void:
	...
	if result and result.collider.has_method("take_damage"):
		...
		# 触发吸血等攻击特效
		if weapon != null and weapon.affixes.has("lifesteal"):
			var ls = weapon.affixes.get("lifesteal", 0)
			if ls > 0 and stats and stats.has_method("heal"):
				stats.heal(ls)
				notification_requested.emit("[嗜血] 回复气血: +" + str(ls))
...
func _execute_spell_cast(is_left: bool, spell_data: SpellData) -> void:
	if spell_data == null: return
	...
	var stats = player.get("stats")
	var base_mana_cost = spell_data.mana_cost
	var element = spell_data.effect_type if spell_data.effect_type != "" else "fire"
	...
	if player.has_method("_spawn_spell_projectile"):
		...
		spell_data.set_meta("_cached_dmg_mult", stats.get_multiplier(element + "_dmg_mult", 1.0) if stats and stats.has_method("get_multiplier") else 1.0)
