...
func _ready() -> void:
	if spell_data != null:
		speed = spell_data.projectile_speed if spell_data.projectile_speed > 0 else 16.0
		max_range = spell_data.cast_range if spell_data.cast_range > 0 else 30.0
		aoe_radius = spell_data.aoe_radius if spell_data.aoe_radius > 0 else 1.0
		damage = spell_data.base_damage
		
		# 加上从 combat_comp.gd 传来的额外伤害倍率
		if spell_data.has_meta("_cached_dmg_mult"):
			damage = int(damage * spell_data.get_meta("_cached_dmg_mult"))
