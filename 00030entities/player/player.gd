...
# 数据与核心节点
# ==========================================
@onready var stats: Node = $ActorDataTemplate/CombatRuntimeAttr
@onready var inventory_comp: InventoryComponent = $Inventory
@onready var hotbar_comp: InventoryComponent = $Hotbar
@onready var equipment_comp: InventoryComponent = $Equipment
@onready var spells: SpellComponent = $Spells
...
@onready var scanner_comp = $ScannerComponent
@onready var sect_builder = $SectBuilderComp
...
func _ready() -> void:
	...
	if not viewmodel:
		var vm_scene = load("res://00030entities/player/viewmodel.tscn")
		if vm_scene:
...
func _on_hotbar_changed(active_idx: int) -> void:
	...
	if builder:
		if active_item_id == "sect_foundation_token":
			if not builder.is_equipped:
				builder.is_equipped = true
				if builder.prompt_label:
					builder.prompt_label.text = "Sect Builder: Hold L-Click to Build"
		else:
...
func use_active_hotbar_item() -> void:
	...
	if item:
		...
		if ItemEffectDispatcher.use_item(self, item):
...
# 施法系统 (动态生成魔法弹)
# ==========================================
func _spawn_spell_projectile(spell: SpellData, is_left: bool) -> void:
	var proj_scene = load("res://00030entities/player/spell_projectile.tscn")
	if proj_scene == null: return
		
	var proj = proj_scene.instantiate()
	proj.spell_name = spell.name
	proj.color = Color(spell.color) if spell.color != "" else Color.WHITE
...
func _unhandled_input(event: InputEvent) -> void:
	# 状态 1: 如果当前正在 UI 模式，屏蔽所有战斗和游戏内交互指令
	if has_node("/root/UIFocusManager") and get_node("/root/UIFocusManager").is_in_ui():
		return 

	# 状态 2: 纯游戏模式 (CAPTURED) 下分发输入指令
	if event is InputEventMouseButton:
		...
		elif event.is_action_pressed("sys_toggle_debug"):
			if has_node("/root/EventBus"):
				get_node("/root/EventBus").request_open_ui.emit("debug_monitor")
