...
func _ready() -> void:
	...
	if get_node_or_null("Model") == null:
		var model_path = monster_data.model_path
...
func _init_from_database() -> void:
	...
	if MonsterDatabase:
		...
		attack_damage = monster_data.attack_damage
		exp_yield = monster_data.exp_yield
		drop_table = monster_data.drop_table
...
func die() -> void:
	...
	# 给玩家加经验
	if target and target.has_node("ActorDataTemplate/CombatRuntimeAttr"):
		# 假设有增加经验的方法
		pass
