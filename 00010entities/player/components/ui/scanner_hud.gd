...
func _on_scan_triggered(targets: Array, duration: float) -> void:
	...
	for t in targets:
		...
		var icon = Label.new()
		icon.text = result.get("icon", "❓")
		icon.add_theme_font_size_override("font_size", 24)
		icon.add_theme_color_override("font_color", result.get("color", Color.WHITE))
		icon.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
		marker.add_child(icon)
		
		var label = Label.new()
		label.text = result.get("name", "未知")
		label.add_theme_font_size_override("font_size", 12)
		label.add_theme_color_override("font_color", result.get("color", Color.WHITE))
