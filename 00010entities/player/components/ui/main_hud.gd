...
func _process(_delta: float) -> void:
	...
	if player:
		...
		if spell_comp and spell_icon_label:
			...
			if not current_spell.is_empty():
				spell_icon_label.text = current_spell.get("icon", "❌")
				spell_name_label.text = current_spell.get("name", "未知")
				
				# 呼吸灯效果
				var t = Time.get_ticks_msec() / 1000.0
				var alpha = 0.7 + 0.3 * sin(t * 3.0)
				var style = spell_icon_panel.get_theme_stylebox("panel") as StyleBoxFlat
				var base_color = current_spell.get("color", Color(0.7, 0.5, 0.2))
...
# --- Casting UI API ---
func show_cast_bar(time_max: float, spell_name: String) -> void:
