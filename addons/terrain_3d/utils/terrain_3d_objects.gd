...
func get_terrain() -> Terrain3D:
	...
	if not terrain or terrain.is_queued_for_deletion() or not terrain.is_inside_tree():
		var terrains: Array[Node] = Engine.get_main_loop().root.get_node(&"EditorInterface").get_edited_scene_root().find_children("", "Terrain3D")
		if terrains.size() > 0:
			terrain = terrains[0]
...
func _is_node_selected(p_node: Node) -> bool:
	var editor_sel = Engine.get_main_loop().root.get_node(&"EditorInterface").get_selection()
