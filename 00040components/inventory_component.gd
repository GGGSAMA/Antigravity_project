...
var slots: Array[ItemStack] = []     # 存放槽位数据的固定大小数组。每个元素为 null 或 Dictionary {"id": String, "qty": int}
...
func next_slot() -> void:
	if size <= 0: return
	var new_idx = (active_slot_index + 1) % size
	set_active_slot(new_idx)

func prev_slot() -> void:
	if size <= 0: return
	var new_idx = (active_slot_index - 1 + size) % size
	set_active_slot(new_idx)
...
func add_item(item_id: String, amount: int = 1, dynamic_data: Dictionary = {}) -> int:
	...
	if not is_unique:
		...
		for i in range(size):
			var data: ItemStack = slots[i]
			if data != null and data.get_item_id() == item_id and data.affixes.is_empty():
				data.qty += remaining
				remaining = 0
	...
	if remaining > 0:
		for i in range(size):
			if slots[i] == null:
				...
				slots[i] = ItemStack.new(meta, drop_qty, str(randi()), dynamic_data.get("quality", 0), dynamic_data.get("affixes", {}).duplicate())
				remaining -= drop_qty
...
func swap_with_component(my_idx: int, other_comp: InventoryComponent, other_idx: int) -> void:
	...
	if my_data != null and other_data != null and my_data.get_item_id() == other_data.get_item_id():
		var is_my_unique = false # Affixes removed from stack level
		var is_other_unique = false # Affixes removed from stack level
...
func remove_item(item_id: String, amount: int = 1) -> int:
	...
	for i in range(size):
		var data: ItemStack = slots[i]
		if data != null and data.get_item_id() == item_id:
