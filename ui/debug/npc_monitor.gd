...
func _ready() -> void:
	...
	close_btn.pressed.connect(func(): visible = false; Input.set_mouse_mode(Input.MOUSE_MODE_CAPTURED))
...
func populate_dropdown() -> void:
	...
	for npc_id in sm.npc_attributes.keys():
		...
		# 如果之前重构没完全把 dict 改成 CharacterData 实例，这里做个兼容
		var n_name = "未知"
...
func refresh_ui() -> void:
	...
	t += "[color=yellow]当前AI状态: %s (锁定 %d 天)[/color]\n\n" % [data.current_action, data.locked_days_remaining]
