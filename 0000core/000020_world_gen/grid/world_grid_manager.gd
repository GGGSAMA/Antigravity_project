...
# 
# [设计意图 / Design Intent]:
# 承担从 3D 无缝大世界到 2D 逻辑数据网格的“降维”映射。
# 它是所有区域加载、宗门领地划分、地图资源生成的核心空间索引。
#
# [架构职责 / Responsibilities]:
# 1. 提供 3D 世界坐标到 2D 逻辑网格坐标的双向换算算法。
# 2. 作为地形高度 (Terrain3D) 的安全抽象层 (Terrain Seam)，提供容错的地形高度获取接口。
# 3. 维护全局所有被占领或探索的 WorldTile 状态，并使用 EventBus 或 Signal 广播领地变更。
# ==============================================================================
extends Node
## -----------------------------------------------------------------------------
## WorldGridManager - 开放世界大网格底层数据单例
...
## 安全获取 3D 坐标对应的真实地形高度
## @param world_pos: 查询的世界 3D 坐标
## @return: 返回真实高度。如果获取失败或超出版图，返回 -9999.0
## @description:
## 封装了对 Terrain3D 插件内存数据的访问。通过返回 -9999.0 的非法值，
## 防止外层将 Terrain3D 尚未初始化的 NaN 错误解析为 0.0 (海平面)，
## 从而避免建筑生成在地下或半空。
func get_terrain_height(world_pos: Vector3) -> float:
	if terrain and "data" in terrain and terrain.data != null:
		...
		if not is_nan(h):
			return h
	# 如果超出 Terrain3D 数据范围（NaN）或没有地形，返回一个极低的非法值，
	# 防止上层生成器误认为这是合法平地 (Y=0.0) 而把物体生成在无限延伸的背景地形下方。
	return -9999.0
