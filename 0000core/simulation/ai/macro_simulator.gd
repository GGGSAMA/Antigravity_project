...
func _on_time_advanced(skipped_hours: float) -> void:
	...
	for npc in sm.npc_attributes.values():
		...
		if best_action:
			# 状态发生变化时，记录到 F4 履历，作为可被玩家观测的里程碑
			if data.current_action != best_action.action_id:
				data.history_trajectory.append({
					"age": data.age,
					"text": "静极思动，决定开始【%s】。" % best_action.action_name,
					"level": 2,
					"type": "routine"
				})
				
				# 限制履历条数，避免溢出
				if data.history_trajectory.size() > 50:
					data.history_trajectory.pop_front()
