...
func seed_initial_sects(count: int = -1) -> void:
	...
	while seeded_count < final_count and attempts < 100:
		...
		if terrain and "data" in terrain and terrain.data:
			var h_center = terrain.data.get_height(Vector3(random_x, 0, random_z))
			
			if is_nan(h_center):
				_log_trace("Skip: is_nan(h_center) at " + str(random_x) + ", " + str(random_z))
				continue
				
			# 如果高度太低（比如在深海/深坑里），放弃
			if h_center < -100.0:
				_log_trace("Skip: h_center < -100.0 at " + str(h_center))
				continue
				
			# 半径 50 米平坦度检查（东西南北4个点）
			var r = 50.0
			var h_n = terrain.data.get_height(Vector3(random_x, 0, random_z - r))
			var h_s = terrain.data.get_height(Vector3(random_x, 0, random_z + r))
			var h_e = terrain.data.get_height(Vector3(random_x + r, 0, random_z))
			var h_w = terrain.data.get_height(Vector3(random_x - r, 0, random_z))
			
			if is_nan(h_n) or is_nan(h_s) or is_nan(h_e) or is_nan(h_w):
				_log_trace("Skip: flat check hit NaN")
				continue
