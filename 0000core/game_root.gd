...
func _ready() -> void:
	...
	if not root.has_node("Log"):
		var log_script = load("res://0000core/log_manager.gd")
		if log_script:
	...
	# 初始化全局 NPC 监控面板
	var monitor_script = load("res://00030ui/debug/npc_monitor.gd")
	if monitor_script:
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventKey and event.pressed and not event.echo:
		if event.keycode == KEY_F1:
			...
			load_level("res://00042world/maps/terrain_level.tscn")
...
func _spawn_fbx_test_nodes(level_node: Node, player_pos: Vector3) -> void:
	...
	var rogue_scene = load("res://00042world/fbx/rogue_all.fbx")
	if rogue_scene:
	...
	var grass_scene = load("res://00042world/fbx/SM_ENV_PLANT_grass_village.fbx")
	if grass_scene:
...
func _spawn_test_npcs(player_pos: Vector3) -> void:
	var npc_scene = load("res://00010entities/npc/npc_base.tscn")
	if not npc_scene:
...
func get_or_create_mystic_realm() -> Vector3:
	if not mystic_realm_instance:
		...
		var realm_scene = load("res://00042world/maps/mystic_realm/mystic_realm_room.tscn")
		if realm_scene:
			...
			push_error("[GameRoot] 无法加载小秘境场景 res://00042world/maps/mystic_realm/mystic_realm_room.tscn")
