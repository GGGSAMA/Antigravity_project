...
static func execute_action(npc: CharacterData, action_dict: Dictionary, time_left_years: float) -> float:
	...
	match action_name:
		...
		"打猎":
			return _execute_hunt(npc, time_left_years)
		"社交结网":
...
static func _execute_hunt(npc: CharacterData, time_left_years: float) -> float:
	var time_spent = min(randf_range(0.2, 1.0), time_left_years) # 打猎耗时短
	
	# 【大世界事务链】发起事务单据
	var processors: Array[StepProcessor] = [
		HuntProcessors.TargetSelectionProcessor.new(),
		HuntProcessors.CombatSimulationProcessor.new(),
		HuntProcessors.SettlementProcessor.new()
	]
	var ticket = TransactionTicket.new(npc.npc_id, "hunt", processors)
