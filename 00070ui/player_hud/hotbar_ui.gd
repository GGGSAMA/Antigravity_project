...
func _on_slot_hover(idx: int):
	...
	if item:
		var meta = ItemDatabase.get_item(item.get_item_id())
		var text = "[color=#F0D050][b]" + str(meta.name if meta.name != "" else item.get_item_id()) + "[/b][/color]\n"
		...
		if event.button_index == MOUSE_BUTTON_LEFT:
			...
			if manager and manager.inventory_panel.visible:
				...
				if manager.carried_item == null:
					...
					if data == null:
						hotbar_comp.set_slot(idx, manager.carried_item.get_item_id(), manager.carried_item.qty, {"affixes": manager.carried_item.affixes, "quality": manager.carried_item.quality})
						manager.carried_item = null
					else:
						if data.get_item_id() == manager.carried_item.get_item_id():
							hotbar_comp.set_slot(idx, data.get_item_id(), data.qty + manager.carried_item.qty, {"affixes": manager.carried_item.affixes, "quality": manager.carried_item.quality})
							manager.carried_item = null
						else:
							var temp = data
							hotbar_comp.set_slot(idx, manager.carried_item.get_item_id(), manager.carried_item.qty, {"affixes": manager.carried_item.affixes, "quality": manager.carried_item.quality})
