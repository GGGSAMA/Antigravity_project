...
func _ready():
	...
	if log_file_full:
		...
		ai_trace("System", "AI-Trace Log Initialized. Ready to record deep state dumps.")
	else:
		print("无法创建日志文件!")
...
func ai_trace(category: String, json_data: String):
	if log_file_ai_trace:
		var time_str = Time.get_time_string_from_system()
		var msg = "%s [AI_TRACE] [%s] %s" % [time_str, category, json_data]
		log_file_ai_trace.store_line(msg)
